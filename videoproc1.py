...
class VideoProcessor:
    def __init__(self, video_file, target_width, target_height, num_animals):
        ...
        self.load_json_data()  # İlk veri yükleme işlemi
        self.cornerspixel = {
    ...
    def load_json_data(self):
        # JSON dosyasını oku ve gerekli verileri al
        ...
        self.corners = {
            ...
            'top_right': tuple(self.camera_coords[1]),
            'bottom_left': tuple(self.camera_coords[2]),
            'bottom_right': tuple(self.camera_coords[3])
        }

        self.animals = self.create_animals()

    def create_animals(self):
        # Animalları yeniden oluştur
        animals = []
        for i in range(self.num_animals):
            animal_name = f'Animal{i+1}'
            random_animal_key = f'animal_coords_{random.randint(1, len(self.animals_coords))}'
            random_animal = self.animals_coords[random_animal_key]
            animal = {
                'name': animal_name,
                'size': 20,
    ...
    def process_video(self):
        while self.cap.isOpened():
            ...
            # Verileri sürekli güncel tut
            self.load_json_data()  # Her karede JSON verisini yeniden yükle
